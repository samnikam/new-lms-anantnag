...
The API needs an always-on process — it runs a Socket.IO gateway (the
studio→classroom broadcast channel) and a `@Cron` device-heartbeat sweep, and
writes uploads to disk. Serverless platforms cannot host it: functions are
stateless, short-lived and have a read-only filesystem, so the broadcast relay
and the device-status board would break.

### Option A — everything on Render (single platform)

`render.yaml` is a complete blueprint: PostgreSQL, the API container and the
static portal, created together in one project.

1. Push the repo to GitHub.
2. On [render.com](https://render.com): **New → Blueprint**, select the repo.
   Render reads `render.yaml` and provisions all three services. The database
   URL and the portal's `VITE_API_URL` are wired automatically.
3. After the first deploy, set one variable by hand on the **API** service:
   `CORS_ORIGIN` = the portal's URL (e.g. `https://lms-portal-web.onrender.com`).
   A blueprint cannot fill this in, because the two services would otherwise
   reference each other in a cycle. Redeploy the API afterwards.
4. Seed the demo data once, from the API service's shell:
...
### Option B — portal on Vercel, API on Render

Use this if you want Vercel's CDN and preview deployments for the portal.

- **Vercel:** import the repo, set **Root Directory** to `frontend`, and add
  `VITE_API_URL` = the Render API URL. `frontend/vercel.json` handles SPA
  rewrites and cache headers. Redeploy after setting the variable — Vite inlines
  `VITE_*` at build time.
- **Render:** deploy only the API service from `render.yaml`, and set
  `CORS_ORIGIN` to the Vercel URL. Set `ALLOW_VERCEL_PREVIEWS=true` to let
  `*.vercel.app` preview builds through CORS.

### Cross-origin notes

In both options the portal and API are separate origins, so the refresh-token
cookie travels in a third-party context. The backend sets `SameSite=None; Secure`
when `NODE_ENV=production`, and `app.set('trust proxy', 1)` lets Express see the
upstream TLS termination. Without both, sign-in appears to succeed but the
session is lost on reload.

`VITE_API_URL` accepts either a full origin or a bare hostname — Render supplies
the latter when the value is wired with `property: host`.

### Free-tier caveats

- **Free web services sleep** after ~15 minutes idle; the next request takes
  ~50 seconds. The portal looks slow on first load, not broken.
- **No persistent disk on the free plan**, so `UPLOAD_DIR` points at `/tmp` and
  uploaded files are lost on redeploy. Attach a paid disk, or move
  `UploadsController` to S3-compatible storage, before real use.
- **Free PostgreSQL instances expire.** Take a `pg_dump` before the expiry date
  if the data matters.
