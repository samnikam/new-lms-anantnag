...
The portal is a static SPA and deploys to **Vercel**. The API does **not** —
it runs a Socket.IO gateway (the studio→classroom broadcast channel), a `@Cron`
device-heartbeat sweep, and writes uploads to disk. Vercel functions are
stateless, short-lived and have a read-only filesystem, so all three break
there. The API therefore runs on an always-on container host.

| Piece | Host | Config |
| --- | --- | --- |
| Portal | Vercel | `frontend/vercel.json`, root directory `frontend` |
| API | Render (or Railway / Fly) | `render.yaml`, `backend/Dockerfile` |
| Database | Neon (or Supabase / Vercel Postgres) | `DATABASE_URL` |
...
Push the repo to GitHub, then **New → Blueprint** on
[render.com](https://render.com) and point it at the repo; `render.yaml` is
picked up automatically. Set the two `sync: false` variables in the dashboard:

- `DATABASE_URL` — the Neon string
- `CORS_ORIGIN` — your Vercel URL, e.g. `https://lms-portal.vercel.app`

`JWT_ACCESS_SECRET` and `JWT_REFRESH_SECRET` are generated by Render. Migrations
run automatically on boot (`prisma migrate deploy` in the Dockerfile `CMD`).

To seed the demo data once the service is live, run from the Render shell:
...
- **Environment variable:** `VITE_API_URL` = your Render URL
  (e.g. `https://lms-portal-api.onrender.com`) — no trailing slash, no `/api`

Redeploy after setting the variable: Vite inlines `VITE_*` at build time, so a
build made before the variable existed will still point at the wrong origin.

### Cross-origin notes

Because the portal and API sit on different domains, the refresh-token cookie
travels in a third-party context. The backend sets `SameSite=None; Secure` when
`NODE_ENV=production`, and `app.set('trust proxy', 1)` lets Express see the
upstream TLS termination — without both, sign-in appears to work but the session
is lost on reload. `ALLOW_VERCEL_PREVIEWS=true` additionally allows
`*.vercel.app` preview deployments through CORS.

### Free-tier caveats

- **Render free services sleep** after ~15 minutes idle; the first request then
  takes ~50 seconds. The portal will look slow on first load, not broken.
- **No persistent disk on the free plan**, so `UPLOAD_DIR` points at `/tmp` and
  uploaded files are lost on redeploy. Move to a paid disk or S3-compatible
  storage before real use.
- **Neon free tier** also suspends idle compute, adding a few seconds to the
  first query.
